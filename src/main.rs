...
const ALL_WORDS: &str = include_str!("../corncob_lowercase.txt");
...
impl WordleGame {
    ...
    fn display_invalid_letters(&self) {
        if !self.guessed_letters.is_empty() {
            println!("Letters not in the word: ");
    ...
    fn ask_for_guess(&mut self) -> String {
        ...
        while !valid_guess {
            ...
            if guess.len() != WORD_LENGTH {
                ...
                println!("{}", "{guess} isn't in the Wordle dictionary.".red())
            } else {
